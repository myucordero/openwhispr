...
- [x] CUDA WhisperX smoke test PASS on WSL AND NATIVE WINDOWS (real 14.5-min recording, large-v3-turbo float16 batch4 + alignment → transcript_complete; WSL RTF 0.170 cold / 0.032 warm; NATIVE WINDOWS RTF 0.173 cold incl. model download / packaged-layout 0.041 warm; 239 segments; canonical transcript validates; native run used Windows Python 3.12.13 + torch 2.8.0+cu128 with Windows paths throughout. Native provisioning surfaced torch+cpu resolution on win32 — fixed via pytorch cu128 index, D-112. Earlier WSL runs surfaced ffprobe-less probe / offline consent env / worker PATH bugs, all fixed.)
- [x] Diarization smoke test PASS (real pyannote/speaker-diarization-community-1 with the user's read-only HF token, WSL CUDA: diarizationUsed=true, 4 speakers SPEAKER_00..03 with plausible turn attribution on the real 14.5-min recording, speaker-transcript artifact produced, canonical transcript validates, RTF 0.080 incl. diarization. The smoke exposed and fixed two real defects: DiarizationPipeline kwargs drift in whisperx 3.8.6 (use_auth_token removed; now model_name/token/device/cache_dir with explicit community-1) and an exception handler that swallowed the failure reason invisibly. Token flowed env-only; never in argv/logs/artifacts.)
- [x] Offline rerun PASS on WSL (allowModelDownload=false, HF_HUB_OFFLINE=1, cached models → transcript_complete RTF 0.032)
- [x] Windows packaged/unpacked build PASS (npm run build:local:win on C:\dev\openwhispr → dist/win-unpacked/OpenWhispr.exe; resources/whisperx-sidecar staged per extraResources allowlist — pyproject/uv.lock/src, tests+venv excluded; 37 binaries in resources/bin; staged .env contains no secrets; packaged-layout smoke: worker executed FROM dist/win-unpacked/resources/whisperx-sidecar with the user-cache runtime → transcript_complete, transcript validates — no repo-relative paths, no dev server)
