...
- [x] Diarization smoke test BLOCKED on user credentials only (requires the user's HF account: accept pyannote/speaker-diarization-community-1 terms + create a read-only token, paste in Settings → WhisperX; everything else is in place — mocked pipeline + fake-worker coverage green)
